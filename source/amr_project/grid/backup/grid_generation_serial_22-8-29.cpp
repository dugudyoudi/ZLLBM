...
/**
* @file grid_generation_manage_serial.cpp
...
* @param[in]  vec_geo_info vetor containing pointers
*             to instance storing geometry information.
* @note
*/
void GridManager::GenerateGridSerial(
...
* @param[in]  ptr_geo_info pointer to instance storing geometry information.
...
* @note
*/
void GridManager::SearchingForNodesBasedOnGeometry(
    ...
    if (ptr_geo_info->node_type_.empty()) {
        ...
        FindTrackingNodesBaseOnGeoPoints(i_geo, 
            i_level, ptr_geo_info->i_level_,
            ptr_geo_info->vec_points_coordi_,
            ptr_map_sfbitset_at_lower_level,
            &(ptr_vec_map_tracking_nodes->at(ptr_geo_info->i_level_)));
    }

    // create layers near the geometry to classify regions of different types
    if (ptr_geo_info->geometry_enclosed_type_
        != criterion::EGeometryEnclosedType::kOpen) {
        IndentifyTypeOfLayerByFloodFill(i_level, i_geo,
            ptr_geo_info, ptr_map_sfbitset_at_lower_level);
    }
}

/**
* @brief   function to find tracking nodes based on geometry points.
* @param[in]  i_geo index of the geometry (only for write log).
* @param[in]  i_level current refinement level.
...
*/
void GridManager::FindTrackingNodesBaseOnGeoPoints(const DefSizet i_geo,
    ...
    void(GridManager:: * ptrfunc_find_tracking_nodes_containing_geo_point)(
        const std::vector<DefReal>&, const std::vector<DefReal>&,
        std::vector<DefSFBitset>*) = nullptr;
    if (k0GridDims_ == 2) {
#ifndef  DEBUG_DISABLE_2D_FUNCTIONS
        ptrfunc_find_tracking_nodes_containing_geo_point =
            &GridManager::FindTrackingNodesContainingAGeometryCoordinate2D;
#endif  // DEBUG_DISABLE_2D_FUNCTIONS
    } else if (k0GridDims_ == 3) {
#ifndef  DEBUG_DISABLE_3D_FUNCTIONS
        ptrfunc_find_tracking_nodes_containing_geo_point =
            &GridManager::FindTrackingNodesContainingAGeometryCoordinate3D;
#endif  DEBUG_DISABLE_3D_FUNCTIONS
    }

    // find tracking nodes based on gometry points
    ...
    for (auto iter = vec_geo_coordi.begin();
        ...
        for (auto iter_sfbitset : vec_node_tracking_lower_level) {
    ...
    int status = CheckIfPointOutsideDomain(coordinate_min, coordinate_max,
...
/**
* @brief   function to add layers near geometry points.
* @param[in]  i_geo index of the geometry (only for write log).
* @param[in]  ptr_geo_info pointer to instance storing geometry information.
...
*                  nodes withbit_sets at the lower level.
*/
void GridManager::IndentifyTypeOfLayerByFloodFill(
    const DefSizet i_level, const DefSizet i_geo,
    const std::shared_ptr<criterion::GeometryInfoInterface> ptr_geo_info,
    DefMap<DefUint>* const ptr_map_sfbitset) {
    ...
    DefUint flag_bit_colored = kFlag0_;
    switch (ptr_geo_info->geometry_enclosed_type_) {
    case criterion::EGeometryEnclosedType::kEnclosedExtend:
        flag_bit_colored = kFlagBitEnclosedExtend_;
        break;
    case criterion::EGeometryEnclosedType::kEnclosedRemove:
        flag_bit_colored = kFlagBitEnclosedRemove_;
        break;
    default:
        break;
    }
    ...
    // set cooridate for finding flood fill starting point the same
    ...
    if (k0GridDims_ == 2) {
        sfbitset_start = FindStartingPointForFloodFill2D(
            i_level, i_geo, flood_fill_origin, *ptr_map_sfbitset);
    } else if (k0GridDims_ == 3) {
        sfbitset_start = FindStartingPointForFloodFill3D(
            i_level, i_geo, flood_fill_origin, *ptr_map_sfbitset);
    }
    FloodFill(sfbitset_start, flag_bit_colored,
        ptr_map_sfbitset, &map_nodes_uncolored, &map_nodes_colored);

    // create layer inside the geometry
}
/**
...
*/
void GridManager::ExtendSameNumberOfLayer(const DefSizet i_level,
    ...
    if (k0GridDims_ == 2) {
        ...
        for (auto iter = map_tracking_node_temp.begin();
            ...
            sfbitset_at_lower_level =
                SFBitsetToNLowerLevel(1, iter->first);
            ...
            std::vector<DefLUint> indices(k0GridDims_, 0);
            SFBitsetComputeIndices2D(sfbitset_at_lower_level, &indices);
            DefSFBitset sfbitset_y = SFBitsetEncoding2D(
...
/**
* @brief   function to do flood fill on existing nodes (2D)
* @param[in]  sfbitset_start   sfbitset corresponding to the starting point.
* @param[in]  flag_bit_colored set flag for colored nodes.
...
*/
void GridManager::FloodFill(const DefSFBitset& sfbitset_start,
    const DefUint flag_bit_coloered,
    ...
    while (!vec_sfbitset_stk.empty() && i < imax_flood_fill) {
        DefSFBitset sfbitset_seed = vec_sfbitset_stk.back();
        vec_sfbitset_stk.pop_back();    
        ++i;  // count repeta
        if ((ptr_map_nodes_exist->find(sfbitset_seed)
            == ptr_map_nodes_exist->end())
            || ((ptr_map_nodes_exist->at(sfbitset_seed) & flag_bit_coloered)
                 == flag_bit_coloered)) {
        } else {
            // color the node and add it to ptr_map_nodes_colored
            ptr_map_nodes_exist->at(sfbitset_seed) |= flag_bit_coloered;
    ...
    for (auto iter = ptr_map_nodes_exist->begin();
        iter != ptr_map_nodes_exist->end(); ++iter) {
...
*                  nodes colored by the flood fill method.
*/
void GridManager::GenerateNodeLayerByLayer(const DefSizet i_level,
    ...
    bool(GridManager:: * ptrfunc_add_nodes_around_a_given_node)(
        const DefSFBitset & sfbitset_in,
    ...
    if (k0GridDims_ == 2) {
        ...
        ptrfunc_add_nodes_around_a_given_node =
            &GridManager::AddNodesAroundAGivenNode2D;
#endif  // DEBUG_DISABLE_2D_FUNCTIONS
    } else if (k0GridDims_ == 3) {
#ifndef  DEBUG_DISABLE_3D_FUNCTIONS
        ptrfunc_check_if_node_not_on_domain_boundary =
        ...
        ptrfunc_add_nodes_around_a_given_node =
            &GridManager::AddNodesAroundAGivenNode3D;
#endif  DEBUG_DISABLE_3D_FUNCTIONS
    }
...
#ifndef  DEBUG_DISABLE_2D_FUNCTIONS
/**
* @brief   function to find starting point for flood fill (2D)
* @param[in]  i_level   level of refinement.
* @param[in]  i_geo     index of the geometry (log write only).
* @param[in]  vec_origin origin coorfinates to search node for flood fill.
* @param[in]  map_nodes_exist nodes exist for flood fill.
* @return     sfbitset corresponding to the starting point.
*/
DefSFBitset GridManager::FindStartingPointForFloodFill2D(
    const DefSizet i_level, const DefSizet i_geo,
    ...
    if (vec_origin.size() != 2) {
        io::LogError("The dimension of vec_origin should be 2 rather than "
            ...
            + std::to_string(i_geo) + " in FindStartingPointForFloodFill2D.");
    }
    ...
    DefSFBitset sfbitset_origin_point =
        SFBitsetEncoding2D({ x_index , y_index });
    // search in -x direction from the vec_origin untill meet the first point
    // in map_nodes_exist
    DefSFBitset sfbitset_temp = sfbitset_origin_point, sfbitset_start_point;
    while (i_count < x_index) {
       sfbitset_temp = FindXNeg2D(sfbitset_temp);
        if (map_nodes_exist.find(sfbitset_temp) != map_nodes_exist.end()) {
            sfbitset_start_point = sfbitset_temp;
    ...
    // search in -y direction from the vec_origin untill meet the first point
    // in map_nodes_exist
    if (bool_find_node_for_flood_fill) {
        sfbitset_temp = sfbitset_origin_point;
        i_count = 0;
...
*/
void GridManager::PushBackSFBitsetInFloodFill2D(const DefSFBitset& sfbitset_in,
...
*/
bool GridManager::AddNodesAroundAGivenNode2D(
    const DefSFBitset& sfbitset_in,
    const std::vector<bool>& vec_bool_need_add_neg,
    const std::vector<bool>& vec_bool_need_add_pos,
...
* @param[in]  vec_origin origin coorfinates to search node for flood fill.
* @param[in]  map_nodes_exist nodes exist for flood fill.
* @return     bit_set corresponding to the starting point.
*/
DefSFBitset GridManager::FindStartingPointForFloodFill3D(
    const DefSizet i_level, const DefSizet i_geo,
    const std::vector<DefReal>& vec_origin,
    const DefMap<DefUint>& map_nodes_exist) {
#ifdef DEBUG_CHECK_GRID
    if (vec_origin.size() != 3) {
        io::LogError("The dimension of vec_origin should be 3 rather than "
            + std::to_string(vec_origin.size()) + "for geometry: "
            + std::to_string(i_geo) + " in FindStartingPointForFloodFill3D.");
    }
#endif  // DEBUG_CHECK_GRID
    // calculate bounds of searching step based on domain boundary
    DefLUint scale_i_level = TwoPowerN(static_cast<DefLUint>(i_level));
    ...
    bool bool_find_node_for_flood_fill = false;
    DefUint i_count = 0, count_sum = 0;;
    DefSFBitset sfbitset_origin_point = SFBitsetEncoding3D(
    ...
    count_sum += i_count;
    // search in -z direction from the vec_origin untill meet the first point
    // in map_nodes_exist
    if (bool_find_node_for_flood_fill) {
       sfbitset_temp = sfbitset_origin_point;
        i_count = 0;
...
* @param[out]  ptr_vec_stk     stack to store seeds.
*/
void GridManager::PushBackSFBitsetInFloodFill3D(const DefSFBitset& sfbitset_in,
    std::vector<DefSFBitset>* const ptr_vec_stk) {
    // add neighbouring nodes to seed
...
* @return     if exists directions in which node does not need to be added 
*/
bool GridManager::AddNodesAroundAGivenNode3D(
    const DefSFBitset& sfbitset_in,
    const std::vector<bool>& vec_bool_need_add_neg,
    const std::vector<bool>& vec_bool_need_add_pos,
    DefMap<DefLUint>* const ptr_map_nodes_exist,
