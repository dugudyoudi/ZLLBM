...
int GridInfoInterface::InterpolationLinear2D(const DefAmrIndexLUint region_length,
    const DefSFBitset& sfbitset_in, const SFBitsetAux2D& sfbitset_aux,
    ...
    if (sfbitset_aux.SFBitsetToNLowerLevel(1, sfbitset_in)
        ...
        for (auto iy = 0; iy <= 1; ++iy) {
            ...
            for (auto ix = 0; ix <= 1; ++ix) {
                ...
                if (nodes_coarse.find(sfbitset_coarse_region.at(index_x)) != nodes_coarse.end()) {
                    NodeInfoCoarse2fine(*nodes_coarse.at(sfbitset_coarse_region.at(index_x)), &node_coarse2fine);
                    *ptr_node += node_coarse2fine * (coeffi_x[ix] * coeffi_y[iy]);
                } else {
                    sfbitset_fine =  sfbitset_aux.SFBitsetToNHigherLevel(1, sfbitset_coarse_region.at(index_x));
                    if (nodes_fine.find(sfbitset_fine) != nodes_fine.end()) {
                        *ptr_node += (*nodes_fine.at(sfbitset_fine).get()) * (coeffi_x[ix] * coeffi_y[iy]);
                    } else {
                        std::array<DefReal, 2> coordinates, grid_space =
                            {grid_space_.at(kXIndex), grid_space_.at(kYIndex)};
                        sfbitset_aux.SFBitsetComputeCoordinate(sfbitset_fine, grid_space, &coordinates);
...
int GridInfoInterface::InterpolationLagrangian2D(const DefAmrIndexLUint interpolation_length,
    ...
    const SFBitsetAux2D& sfbitset_aux, const std::vector<DefSFBitset>& sfbitset_coarse_region,
    const DefMap<std::unique_ptr<Node_T>>& nodes_fine, const DefMap<std::unique_ptr<Node_T>>& nodes_coarse,
    Node_T* const ptr_node) {
#ifdef DEBUG_CHECK_GRID
    // check if the input space filling code of one level lower is the same as that of the region center
...
int GridInfoInterface::InterpolationLinear3D(const DefAmrIndexLUint region_length,
    const DefSFBitset& sfbitset_in, const SFBitsetAux3D& sfbitset_aux,
    const std::vector<DefSFBitset>& sfbitset_coarse_region,
    const DefMap<std::unique_ptr<Node_T>>& nodes_fine, const DefMap<std::unique_ptr<Node_T>>& nodes_coarse,
    Node_T* const ptr_node) {
#ifdef DEBUG_CHECK_GRID
                ...
                for (auto ix = 0; ix <= 1; ++ix) {
                    ...
                    if (nodes_coarse.find(sfbitset_coarse_region.at(index_x)) != nodes_coarse.end()) {
                        ...
                        *ptr_node += node_coarse2fine * (coeffi_x[ix] * coeffi_y[iy]* coeffi_z[iz]);
                        ...
                        sfbitset_fine =  sfbitset_aux.SFBitsetToNHigherLevel(1, sfbitset_coarse_region.at(index_x));
                        if (nodes_fine.find(sfbitset_fine) != nodes_fine.end()) {
                            *ptr_node += (*nodes_fine.at(sfbitset_fine).get())*(coeffi_x[ix]*coeffi_y[iy]*coeffi_z[iz]);
                        } else {
                            std::array<DefReal, 3> coordinates, grid_space =
...
int GridInfoInterface::InterpolationLagrangian3D(const DefAmrIndexLUint interpolation_length,
    const DefAmrIndexLUint region_length, const DefSFBitset& sfbitset_in,
    const SFBitsetAux3D& sfbitset_aux, const std::vector<DefSFBitset>& sfbitset_coarse_region,
    const DefMap<std::unique_ptr<Node_T>>& nodes_fine, const DefMap<std::unique_ptr<Node_T>>& nodes_coarse,
    Node_T* const ptr_node) {
#ifdef DEBUG_CHECK_GRID
    // check if the input space filling code of one level lower is the same as that of the region center
