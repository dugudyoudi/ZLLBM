        ...
        if (ptr_interface_info_lower->vec_inner_coarse2fine_.size() > 0) {
            ...
            for (int ilayer = 0; ilayer < maxlayer; ++ilayer) {
                if (ilayer == maxlayer - 1) {
                    ...
                    flag_temp = flag_0 | NodeBitStatus::kNodeStatusFine2CoarseGhost_;
                } else {
                    flag_temp = flag_0;
                }
            ...
            for (DefAmrIndexUint ilayer = 0; ilayer < maxlayer; ++ilayer) {
                if (ilayer == maxlayer - 1) {
                    ...
                    flag_temp = flag_0 | NodeBitStatus::kNodeStatusCoarse2FineGhost_;
                } else {
                    flag_temp = flag_0;
                }
...
void GridManagerInterface::InstantiateGridNodeAllLevelMpi(const int i_rank,
    ...
    for (DefAmrIndexUint i_level = k0MaxLevel_; i_level > 0; --i_level) {
        ...
        InstantiateDomainBoundary(num_partition_outer_layer - 1, code_min_background_level,
        ...
        for (const auto& iter_interface : partition_interface_level) {
            ...
            sfbitset_aux.FindNodesInPeriodicReginNearby(iter_interface.first, num_partition_outer_layer,
                periodic_min, periodic_max, domain_min_n_level, domain_max_n_level, &vec_in_region);
            for (const auto& iter_node : vec_in_region) {
                code_tmp = sfbitset_aux.SFBitsetToNLowerLevelVir(i_level, iter_node).to_ullong();
                if ((code_tmp < code_min_background_level || code_tmp > code_max_background_level)
        ...
        if (i_level < k0MaxLevel_) {
            SetUpInnerLayerForHigherLevel(i_rank, i_level, num_partition_outer_layer,
    ...
    if (k0MaxLevel_ > 0) {
        SetUpInnerLayerForHigherLevel(i_rank, 0, num_partition_outer_layer, code_min_background_level,
    ...
    // setup node flag for refinement interface on mpi outer layer
    for (DefAmrIndexUint i_level = 1; i_level <= k0MaxLevel_; ++i_level) {
        DefMap<DefAmrUint> map_lower_level_marked, map_lower_level_tmp;
        std::vector<DefSFBitset> corner_bitsets;
        DefAmrIndexUint layer_fine;
    ...
    //     for (auto iter : ptr_mpi_inner_layer->at(0).at(0)) {
    ...
    //     for (auto iter : ptr_mpi_outer_layer->at(0)) {
    //         amrproject::SFBitsetAux2D aux2d;
    //         std::array<DefAmrIndexLUint, 2> indices;
    //         aux2d.SFBitsetComputeIndices(iter.first, &indices);
    //         file1 << indices[0] << " " << indices[1] << std::endl;
 ...
 */
void  GridManagerInterface::InstantiateDomainBoundary(const DefAmrIndexLUint num_ghost_layer,
        ...
        for (const auto& iter_node : boundary_min_tmp) {
            ...
            if (periodic_min.at(i_dim)) {
                ...
                if (code_tmp >= code_min_background_level && code_tmp <= code_max_background_level) {
                    ...
                    map_grid.at(sfbitset_tmp)->flag_status_ |= NodeBitStatus::kNodeStatusMpiPartitionInner_;
                }
 ...
 */
void GridManagerInterface::SetUpInnerLayerForHigherLevel(int i_rank,
                ...
                if (code_background < code_min_background_level || code_background > code_max_background_level) {
                    if (mpi_outer_layer.find(iter_outer) == mpi_outer_layer.end()) {
                        ...
                        if (map_grid_node_higher_level.find(sfbitset_higher_level)
                            ...
                            node_rank = static_cast<int>(iter_index - ull_max.begin());
                            sfbitset_aux.FindNodesInPeriodicReginNearby(iter_outer, num_partition_outer_layer,
