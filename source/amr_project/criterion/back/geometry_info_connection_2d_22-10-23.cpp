...
void GeometryInfoConnection2D::BisectinOnceLine(
    ...
    DefReal dis;
    GeometryCoordinateIndex vertex_index_temp;
    ...
    for (const auto& iter_edge : vec_edge_for_bisect) {
        ...
        if (dis > ds_max) {  // edge needs to bisect
            ...
            vertex_index_temp.layer_level = i_layer_level;
            vertex_index_temp.vertex_index = i_vertex;
            coordinate_given_level_.at(level_vertex0).vec_vertex_cooridinate
                .at(i_vertex0).vec_child_vertices.push_back(vertex_index_temp);
            coordinate_given_level_.at(level_vertex1).vec_vertex_cooridinate
                .at(i_vertex1).vec_child_vertices.push_back(vertex_index_temp);
...
void GeometryInfoConnection2D::MergeOnceLine(
    ...
    std::set<DefSizet>  edge_remove, edge_remain, edge_remain_for_merge, surface_move;
        ...
        if (dis < ds_min) {  // edge needs to merge
            ...
            if (point_processed.find(point_index_temp)
                ...
                if (vec_edge_others.size() > 0) {
                    ...
                    for (const auto& iter_child : coordinate_given_level_
                        .at(level_remove).vec_vertex_cooridinate
                        .at(vertex_remove).vec_child_vertices) {
                        point_remain.insert(std::make_pair(
                            iter_child.layer_level, iter_child.vertex_index));
                    }
                    ...
                    for (const auto& iter_child : coordinate_given_level_
                        .at(level_remove).vec_vertex_cooridinate
                        .at(vertex_remove).vec_child_vertices) {
                        point_remove.insert(std::make_pair(
                            iter_child.layer_level, iter_child.vertex_index));
                    }
                }
                connection_edge_given_level_.at(i_input_level)
                ...
                if (edge_remain.find(edge_index_min) == edge_remain.end()) {
                    edge_remain.insert(edge_index_min);
                    edge_remove.erase(edge_index_min);
                    if (dis / 2. < ds_min) {
                        edge_remain_for_merge.insert(edge_index_min);
                    }
                }
            }
    ...
    // child vertices for nodes need to be removed
    DefUint iter_count = 0, iter_max = 10;
    std::set<std::pair<DefSizet, DefSizet>> point_remain_temp;
    while (point_remain.size() > 0 && iter_count < iter_max) {
        ++iter_count;
        point_remain_temp = point_remain;
        point_remain.clear();
        for (const auto& iter_point : point_remain_temp) {
    ...
    // find edges need to be removed in edge_remain, i.e. linked two vertices
    // need to be removed
    for (const auto& iter_edge : edge_remain) {
        ...
        if (point_processed.find(std::make_pair(level_vertex0, i_vertex0))
            ...
            if (edge_remain_for_merge.find(iter_edge) != edge_remain_for_merge.end()) {
    ...
    // remove edges
    for (const auto& iter_edge: edge_remove) {
        // delete edge relations in coordinate_given_level_
        level_vertex0 = connection_edge_given_level_.at(i_input_level)
            .vec_edge_connection.at(iter_edge).vertex_connection.at(0)
            .layer_level;
        i_vertex0 = connection_edge_given_level_.at(i_input_level)
            .vec_edge_connection.at(iter_edge).vertex_connection.at(0)
            .vertex_index;
        if (point_remove.find(std::make_pair(level_vertex0, i_vertex0))
            == point_remove.end()) {
            for (auto& iter_index : coordinate_given_level_.at(level_vertex0)
                .vec_vertex_cooridinate.at(i_vertex0).vec_index_edges) {
                if (iter_index.first == i_input_level
                    && iter_index.second == iter_edge) {
                    std::swap(iter_index, coordinate_given_level_
                        .at(level_vertex0).vec_vertex_cooridinate
                        .at(i_vertex0).vec_index_edges.back());
                }
            }
            coordinate_given_level_.at(level_vertex0).vec_vertex_cooridinate
                .at(i_vertex0).vec_index_edges.pop_back();
        }
        level_vertex0 = connection_edge_given_level_.at(i_input_level)
            .vec_edge_connection.at(iter_edge).vertex_connection.at(1)
            .layer_level;
        ...
        // delete edge from connection_edge_given_level_.at(i_input_level)
        num_edge = connection_edge_given_level_.at(i_input_level)
            .vec_edge_connection.size();
        if (iter_edge < num_edge) {
            while (edge_remove.find(iter_edge) != edge_remove.end()
                && num_edge > 0) {
                --num_edge;
                // delete the last element of edge
                connection_edge_given_level_.at(i_input_level)
                    .vec_edge_connection.pop_back();
            }
            if (iter_edge < num_edge) {
                // delete edge
                std::swap(connection_edge_given_level_.at(i_input_level)
                    .vec_edge_connection.at(iter_edge),
                    connection_edge_given_level_.at(i_input_level)
                    .vec_edge_connection.back());
                connection_edge_given_level_.at(i_input_level)
                    .vec_edge_connection.pop_back();
                // update edge relations in coordinate_given_level_
                level_vertex0 = connection_edge_given_level_.at(i_input_level)
                    .vec_edge_connection.at(iter_edge).vertex_connection.at(0)
                    .layer_level;
                i_vertex0 = connection_edge_given_level_.at(i_input_level)
    ...
    for (const auto& iter_point : point_remove) {
        num_points = coordinate_given_level_.at(iter_point.first)
            .vec_vertex_cooridinate.size();
        if (iter_point.second < num_points) {
            while (point_remove.find(std::make_pair(iter_point.first,
                num_points)) != point_remove.end() && num_points > 0) {
                --num_points;
                // delete child relation
                DeleteChildRelation(iter_point.first, num_points);
                // delete coordinates
                coordinate_given_level_.at(iter_point.first)
            ...
            if (iter_point.second < num_points) {
                // delete child relation
                DeleteChildRelation(iter_point.first, iter_point.second);
                // delete coordinatese
                std::swap(coordinate_given_level_.at(iter_point.first)
                    .vec_vertex_cooridinate.at(iter_point.second),
                    coordinate_given_level_.at(iter_point.first)
                    .vec_vertex_cooridinate.at(num_points));
                coordinate_given_level_.at(iter_point.first)
                    .vec_vertex_cooridinate.pop_back();
                // update child relation
                level_vertex0 = coordinate_given_level_.at(iter_point.first)
                    .vec_vertex_cooridinate.at(iter_point.second)
                    .parent_vertices.at(0).layer_level;
                i_vertex0 = coordinate_given_level_.at(iter_point.first)
                    .vec_vertex_cooridinate.at(iter_point.second)
                    .parent_vertices.at(0).vertex_index;
                for (auto& iter_child : coordinate_given_level_
                    .at(level_vertex0).vec_vertex_cooridinate.at(i_vertex0)
                    .vec_child_vertices) {
                    if (iter_child.layer_level == iter_point.first
                        && iter_child.vertex_index == num_points) {
                        iter_child.vertex_index = iter_point.second;
                        break;
                    }
                }
                level_vertex0 = coordinate_given_level_.at(iter_point.first)
                    .vec_vertex_cooridinate.at(iter_point.second)
                    .parent_vertices.at(1).layer_level;
                i_vertex0 = coordinate_given_level_.at(iter_point.first)
                    .vec_vertex_cooridinate.at(iter_point.second)
                    .parent_vertices.at(1).vertex_index;
                for (auto& iter_child : coordinate_given_level_
                    .at(level_vertex0).vec_vertex_cooridinate.at(i_vertex0)
                    .vec_child_vertices) {
                    if (iter_child.layer_level == iter_point.first
                ...
                for (auto& iter_child : coordinate_given_level_
                    .at(iter_point.first).vec_vertex_cooridinate
                    .at(iter_point.second).vec_child_vertices) {
                    if (coordinate_given_level_.at(iter_child.layer_level)
                        .vec_vertex_cooridinate.at(iter_child.vertex_index)
                        .parent_vertices.at(0).layer_level == iter_point.first
                        && coordinate_given_level_.at(iter_child.layer_level)
                        .vec_vertex_cooridinate.at(iter_child.vertex_index)
                        .parent_vertices.at(0).vertex_index == num_points) {
                        coordinate_given_level_.at(iter_child.layer_level)
                            .vec_vertex_cooridinate.at(iter_child.vertex_index)
                            .parent_vertices.at(0).vertex_index
                            = iter_point.second;
                ...
                for (auto& iter_edge : coordinate_given_level_
                    .at(iter_point.first).vec_vertex_cooridinate
                    .at(iter_point.second).vec_index_edges) {
                    if (connection_edge_given_level_.at(iter_edge.first)
                        .vec_edge_connection.at(iter_edge.second)
                        .vertex_connection.at(0).layer_level
                        == iter_point.first
                        && connection_edge_given_level_.at(iter_edge.first)
                        .vec_edge_connection.at(iter_edge.second)
                        .vertex_connection.at(0).vertex_index
                        == num_points) {
                        connection_edge_given_level_.at(iter_edge.first)
...
void GeometryInfoConnection2D::MergeOnceTriangle(
    const DefSizet i_input_level, const DefReal ds_min,
    ...
    std::array<DefReal, 2> coordi0, coordi1;
    DefSizet level_vertex0, level_vertex1, level_remove, vertex_remove;
    std::set<DefSizet>  edge_remove, edge_remain, edge_remain_for_merge,
        surface_reconstruct;
    std::pair<DefSizet, DefSizet> point_index_temp;
    // (point_porcessed) include (point_remove) and points linked to edges at
    // levels other than i_input_level
    std::set<std::pair<DefSizet, DefSizet>> point_processed,
        point_remove, point_remain;
    // find cooridinates and edges may need to be removed according
    // to the input
    for (const auto& iter_edge : vec_edge_for_bisect) {
        level_vertex0 = connection_edge_given_level_.at(i_input_level)
            .vec_edge_connection.at(iter_edge)
    ...
    // child vertices at lower layer levels and their linked edges
    // need to be removed
    DefUint iter_count = 0, iter_max = 10;
    std::set<std::pair<DefSizet, DefSizet>> point_remain_temp;
    while (point_remain.size() > 0 && iter_count < iter_max) {
        ...
        if (point_processed.find(std::make_pair(level_vertex0, i_vertex0))
            ...
            for (const auto& iter_surface : connection_edge_given_level_
                ...
                i_surface = connection_surface_given_level_
                    ...
                    .parent_surface;
                surface_reconstruct.insert(i_surface);
                connection_surface_given_level_
                    .at(i_input_level).vec_surface_connection.at(i_surface)
                    .edge_connection.push_back(iter_edge);
            }
            if (edge_remain_for_merge.find(iter_edge)
                != edge_remain_for_merge.end()) {
                ptr_surface_remain_for_merge->insert(iter_edge);
            }
        }
    }
    ...
    // reconstruct surface
    DefSizet i_edge;
    for (const auto& iter_surface : surface_reconstruct) {
        std::queue<DefSizet> surface_temp, surface_remove;
        std::map<std::pair<DefSizet, DefSizet>, std::vector<DefSizet>>
            point_exist;
        surface_temp.push(iter_surface);

        // find vertices and edges do not need to remove in child surfaces 
        while (!surface_temp.empty()) {
            i_surface = surface_temp.front();
            surface_temp.pop();
            surface_remove.push(i_surface);
            // push child surfaces
            while (!connection_surface_given_level_
                .at(i_input_level).vec_surface_connection.at(i_surface)
                .child_surface.empty()) {
                surface_temp.push(connection_surface_given_level_
                    .at(i_input_level).vec_surface_connection.at(i_surface)
                    .child_surface.back());
                connection_surface_given_level_
                    .at(i_input_level).vec_surface_connection.at(i_surface)
            ...
            i_layer_level = connection_surface_given_level_.at(i_input_level)
                .vec_surface_connection.at(i_surface)
                .vetex_connectin.at(0).layer_level;
            i_vertex = connection_surface_given_level_.at(i_input_level)
                .vec_surface_connection.at(i_surface)
                .vetex_connectin.at(0).vertex_index;
            if (point_remove.find(std::make_pair(i_layer_level, i_vertex))
                == point_remove.end()
                && point_exist.find(std::make_pair(i_layer_level, i_vertex))
                == point_exist.end()) {
                point_exist.insert({ std::make_pair(
                    i_layer_level, i_vertex), {} });
            }
            i_layer_level = connection_surface_given_level_.at(i_input_level)
                .vec_surface_connection.at(i_surface)
                .vetex_connectin.at(1).layer_level;
            i_vertex = connection_surface_given_level_.at(i_input_level)
                .vec_surface_connection.at(i_surface)
                .vetex_connectin.at(1).vertex_index;
            if (point_remove.find(std::make_pair(i_layer_level, i_vertex))
                == point_remove.end()
                && point_exist.find(std::make_pair(i_layer_level, i_vertex))
                == point_exist.end()) {
            ...
            i_layer_level = connection_surface_given_level_.at(i_input_level)
                .vec_surface_connection.at(i_surface)
                .vetex_connectin.at(2).layer_level;
            i_vertex = connection_surface_given_level_.at(i_input_level)
                .vec_surface_connection.at(i_surface)
                .vetex_connectin.at(2).vertex_index;
            if (point_remove.find(std::make_pair(i_layer_level, i_vertex))
                == point_remove.end()
                && point_exist.find(std::make_pair(i_layer_level, i_vertex))
                == point_exist.end()) {
            ...
            while (!connection_surface_given_level_
                .at(i_input_level).vec_surface_connection.at(i_surface)
                .edge_connection.empty()) {
                i_edge = connection_surface_given_level_
                    .at(i_input_level).vec_surface_connection.at(i_surface)
                    .edge_connection.back();
                if (edge_remove.find(i_edge) == edge_remove.end()) {
                    if (connection_edge_given_level_.at(i_input_level)
                        .vec_edge_connection.at(i_edge)
                        .vertex_connection.at(0).layer_level >
                        connection_edge_given_level_.at(i_input_level)
                        .vec_edge_connection.at(i_edge)
                        .vertex_connection.at(1).layer_level) {
                        point_exist.at(std::make_pair(
                            connection_edge_given_level_
                            .at(i_input_level).vec_edge_connection.at(i_edge)
                            .vertex_connection.at(0).layer_level,
                            connection_edge_given_level_.at(i_input_level)
                            .vec_edge_connection.at(i_edge).vertex_connection.
                            at(0).vertex_index)).push_back(i_edge);
                    } else {
                        point_exist.at(std::make_pair(
                            connection_edge_given_level_
                            .at(i_input_level).vec_edge_connection.at(i_edge)
                            ...
                            connection_edge_given_level_.at(i_input_level)
                            .vec_edge_connection.at(i_edge).vertex_connection
                            .at(1).vertex_index)).push_back(i_edge);
                    }
                }
                connection_surface_given_level_
                    .at(i_input_level).vec_surface_connection.at(i_surface)
                    .edge_connection.pop_back();
            }
        }

        // bisect the root triangle for all exist points
        // the root triangle
        connection_surface_given_level_
            .at(i_input_level).vec_surface_connection.at(iter_surface)
        for (const auto& iter_point : point_exist){
...
void GeometryInfoConnection2D::DeleteChildRelation(
    const DefSizet layer_level, const DefSizet index) {
    DefSizet level_vertex0, i_vertex0;
    level_vertex0 = coordinate_given_level_.at(layer_level)
        .vec_vertex_cooridinate.at(index).parent_vertices.at(0)
        .layer_level;
    i_vertex0 = coordinate_given_level_.at(layer_level)
        .vec_vertex_cooridinate.at(index).parent_vertices.at(0)
        .vertex_index;
    for (auto& iter_child : coordinate_given_level_.at(level_vertex0)
        .vec_vertex_cooridinate.at(i_vertex0).vec_child_vertices) {
        if (iter_child.layer_level == layer_level
            && iter_child.vertex_index == index) {
            std::swap(iter_child, coordinate_given_level_
                .at(level_vertex0).vec_vertex_cooridinate
                .at(i_vertex0).vec_child_vertices.back());
            coordinate_given_level_.at(level_vertex0)
                .vec_vertex_cooridinate.at(i_vertex0)
                .vec_child_vertices.pop_back();
            break;
        }
    }
    level_vertex0 = coordinate_given_level_.at(layer_level)
        .vec_vertex_cooridinate.at(index).parent_vertices.at(1)
        .layer_level;
    i_vertex0 = coordinate_given_level_.at(layer_level)
        .vec_vertex_cooridinate.at(index).parent_vertices.at(1)
        .vertex_index;
