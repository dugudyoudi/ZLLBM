...
void GeometryInfoConnection2D::InitialConnectionAddEdge(
    ...
    if (coordinate_given_level_.at(0).vec_vertex_cooridinate
        ...
        for (const auto& iter_edge :
            ...
            if (connection_edge_given_level_.at(0).vec_edge_connection
                .at(iter_edge).vertex_connection[1].vertex_index
                == index_large) {
                bool_exist = true;
                connection_edge_given_level_.at(0).vec_edge_connection
                    .at(iter_edge).vec_index_surfaces.push_back(i_surface);
                connection_surface_given_level_.at(0).vec_surface_connection
                    .at(i_surface).edge_connection.push_back(iter_edge);
                break;
            }
        }
    }
    ...
    if (!bool_exist) {
        ...
        DefSizet iter_edge = connection_edge_given_level_.at(0)
            .vec_edge_connection.size();
        connection_edge_temp.vec_index_surfaces = { i_surface };
        connection_edge_given_level_.at(0).vec_edge_connection
            .push_back(connection_edge_temp);
        coordinate_given_level_.at(0).vec_vertex_cooridinate
            .at(index_small).vec_index_edges.push_back(iter_edge);
        coordinate_given_level_.at(0).vec_vertex_cooridinate
            .at(index_large).vec_index_edges.push_back(iter_edge);
        connection_surface_given_level_.at(0).vec_surface_connection
            .at(i_surface).edge_connection.push_back(iter_edge);
    }
}
void GeometryInfoConnection2D::BisectinOnceLine(const DefSizet i_input_level,
    const DefReal ds_min, const DefReal ds_max,
    ...
    std::vector<DefSizet>::iterator iter_temp;
    DefReal dis;
    ...
    for (const auto& iter_edge : vec_edge_for_bisect) {
        ...
        if (dis > ds_max) {  // edge needs to bisect
            ...
            coordi_connection_temp.vec_index_edges = { iter_edge, num_edge};
            ...
            iter_temp = std::find(coordinate_given_level_.at(level_vertex1)
                .vec_vertex_cooridinate.at(i_vertex1)
                .vec_index_edges.begin(), coordinate_given_level_
                .at(level_vertex1).vec_vertex_cooridinate.at(i_vertex1)
                .vec_index_edges.end(), iter_edge);
            *iter_temp = num_edge;
        ...
        } else if (dis > ds_min) {  // edge need to merge
            if (coordinate_given_level_.at(level_vertex0)
                .vec_vertex_cooridinate.at(i_vertex0).parent_vertices.at(0)
                .layer_level > coordinate_given_level_.at(level_vertex0)
                .vec_vertex_cooridinate.at(i_vertex0).parent_vertices.at(0)
                .layer_level) {
            }
