...
void GeometryInfoInterface::ReadAndSetGeoParameters(const DefInt level,
    ...
    input_parser.GetValue<DefInt>("geo.level", geo_parameters, &geo_level);
    SetLevel(geo_level);
    ...
    input_parser.GetValue<DefInt>("geo.x_extend_pos", geo_parameters, &x_extend_pos);
    SetXExtendPositive(x_extend_pos);
    input_parser.GetValue<DefInt>("geo.x_extend_neg", geo_parameters, &x_extend_neg);
    SetXExtendNegative(x_extend_neg);
    input_parser.GetValue<DefInt>("geo.y_extend_pos", geo_parameters, &y_extend_pos);
    SetYExtendPositive(y_extend_pos);
    input_parser.GetValue<DefInt>("geo.y_extend_neg", geo_parameters, &y_extend_neg);
    SetYExtendNegative(y_extend_neg);
    if (k0GeoDim_ == 3) {
        input_parser.GetValue<DefInt>("geo.z_extend_pos", geo_parameters, &z_extend_pos);
        SetZExtendPositive(z_extend_pos);
        input_parser.GetValue<DefInt>("geo.z_extend_neg", geo_parameters, &z_extend_neg);
        SetZExtendNegative(z_extend_neg);
    }
    std::string extend_type = "same_in_all_directions";
    input_parser.GetValue<std::string>("geo.extend_type", geo_parameters, &extend_type);
